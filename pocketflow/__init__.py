...
class BaseNode:
    ...
    def __rshift__(self,other): return self.next(*(other if isinstance(other,(tuple, set, list)) else [other]))
...
class Node(BaseNode):
    def __init__(self,max_retries=1,wait=0): super().__init__(); self.cur_retry=0;self.max_retries,self.wait=max_retries,wait
